...
export class AddmaterialComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   async addCourse(values:any){
    ...
    if(!values.name ||!values.description){
      ...
      values.courseId=0;
      console.log(values)
      const course:Course=await this.courseService.createCourse(values,token);
      console.log(course)
  ...
  async addCourseMaterial(values:any){
    ...
    const courseMaterials ={materialType:values.materialType,description:values.description}
    this.courseMaterial.push(courseMaterials)
    values.courseId=this.courseId;
    values.id=0
    console.log(values)
    const courseMaterial=await this.courseMaterialService.createCourseMaterial(values,token);
